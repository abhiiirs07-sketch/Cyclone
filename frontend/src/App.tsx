...
function App() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-cyber-dark text-slate-100 select-none">
      ...
      {/* 3. Unified Left Control Panel Sidebar - Desktop Only */}
      {!isMobile && (
        <div
          className={`absolute top-24 bottom-6 z-40 w-[420px] flex flex-col pointer-events-auto transition-all duration-300 glass-panel rounded-xl shadow-2xl ${
            leftSidebarOpen ? 'left-4 translate-x-0' : '-translate-x-[440px] left-0'
          }`}
        >
            ...
            {activeTab === 'historical' && (
              <div className="flex flex-col h-full overflow-hidden">
                ...
                {cycloneTrack.length > 0 && (
                  <div className="h-[220px] shrink-0 flex flex-col p-3.5 border-t border-white/5 bg-slate-950/40 overflow-hidden">
                    <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-white/5 pb-1.5 mb-1.5">
      ...
      {!isMobile && (
        <div className={`absolute bottom-6 z-40 flex flex-col gap-2 pointer-events-auto transition-all duration-300 ${
          leftSidebarOpen ? 'left-[445px]' : 'left-6'
        } right-6`}>
          {cycloneTrack.length > 0 && (
